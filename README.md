#### my projects:

- [CS & ML](https://github.com/stars/katistix/lists/cs-ml)
    - [spam-filtering](https://github.com/katistix/spam-filtering) - Naive Bayes implementation in Go

- [productivity tools](https://github.com/stars/katistix/lists/productivity-tools)
    - [plate](https://github.com/katistix/plate) - TUI for managing your local development databases, written in Go
    - [zenith](https://github.com/katistix/zenith) - A beautifully minimal floating countdown timer that stays on top of everything. (SwiftUI)

- [InfoEducatie](https://github.com/stars/katistix/lists/infoeducatie)
    - [uws](https://github.com/katistix/uws) - 🦄 Unicorn Web Services - InfoEducatie 2025 OPEN


#### dotfiles:
- [katistix/dotfiles](https://github.com/katistix/dotfiles)